...
// Removed deleted app stores

// Stub type for removed IE functionality
type Favorite = { 
  title: string; 
  isDirectory?: boolean; 
  children?: Favorite[];
...
// Video track interface (for removed videos functionality)
...
export function useFileSystem(
  ...
  const {
    videos: videoTracks,
  } = { videos: [] as VideoTrack[] }; // Removed
  const internetExplorerStore = { favorites: [] as Favorite[] }; // Removed
  ...
  const loadFiles = useCallback(async () => {
    ...
    try {
      ...
      if (currentPath === "/Applications") {
        ...
        // Collect all unique artists
        videoTracks.forEach((video: VideoTrack) => {
          if (video.artist) {
        ...
        if (videoTracks.some((video: VideoTrack) => !video.artist)) {
          displayFiles.push({
            name: "Unknown Artist",
            isDirectory: true,
        ...
        const artistVideos = videoTracks.filter((video: VideoTrack) =>
          artistName === "Unknown Artist"
            ? !video.artist
        ...
        displayFiles = artistVideos.map((video: VideoTrack) => {
          return {
        ...
        displayFiles = currentLevelFavorites.map((fav: Favorite) => {
          ...
          return {
            ...
            appId: isDirectory ? undefined : undefined, // Removed IE
      ...
      // b. Video Library (Virtual)
        ...
        displayFiles = videoTracks.map((video: VideoTrack) => ({
          name: `${video.title}.mov`,
          isDirectory: false,
    ...
    async (file: ExtendedDisplayFileItem) => {
      ...
      try {
        ...
        if (file.path.startsWith("/Applications/") && file.appId) {
          ...
          // Paint app removed - show message
          alert("Paint app is not available");
          ...
          console.log("Videos functionality removed");
        } else if (file.type === "site-link" && file.data?.url) {
          // Removed IE functionality - site links no longer functional
          console.log("Site links removed with IE functionality");
        } else {
          console.warn(

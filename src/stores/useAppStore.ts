    ...
    (set, get) => ({
      ...
      launchApp: (appId, initialData, title, multiWindow = false) => {
        ...
        if (!supportsMultiWindow) {
          ...
          const staleInstances = Object.values(state.instances).filter(
            (i) => i.appId === appId && !i.isOpen
          );
          if (staleInstances.length > 0) {
            set((s) => {
              const instances = { ...s.instances };
              const instanceOrder = [...s.instanceOrder];
              
              staleInstances.forEach((inst) => {
                delete instances[inst.instanceId];
                const orderIndex = instanceOrder.indexOf(inst.instanceId);
                if (orderIndex >= 0) {
                  instanceOrder.splice(orderIndex, 1);
                }
              });
              
              return { instances, instanceOrder };
            });
          }
        ...
        const instanceId = state.createAppInstance(appId, initialData, title);
        return instanceId;
      },

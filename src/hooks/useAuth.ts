...
export function useAuth() {
  // Authentication system disabled - all auth functionality is stubbed
  const username = null;
  const authToken = null;
  const hasPassword = false;
  const setAuthToken = () => {};
  const setUsername = () => {};
  const createUser = async () => ({ ok: false, error: "Authentication system is disabled" });
  const logout = async () => {};
  const storeCheckHasPassword = async () => false;
  const storeSetPassword = async () => ({ ok: false, error: "Authentication system is disabled" });
